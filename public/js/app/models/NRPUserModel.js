define([
  ...
  var NRPUserModel = Backbone.Model.extend({
    ...
    initialize: function() {
      ...
      this.url = 'http://'+global.serverIp+':'+global.serverPort+'/api/nrpuser/'+this.id+'/update';
    },
    ...
    save: function() {
      ...
      $.getJSON(this.url, this.attributes, function(data) {
        ...
        //debugger;
        log.push('NRPUserModel.save() executed.');

      }).error( function(err) {
        //This is the error handler.
        //debugger;
        log.push('Error while trying NRPUserModel.save(). Most likely due to communication issue with the server.');
        sendLog();
        console.error('Communication error with server while execute NRPUserModel.save()');
      });
