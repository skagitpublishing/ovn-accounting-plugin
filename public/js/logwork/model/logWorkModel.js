define([
  ...
  var LogWorkModel = Backbone.Model.extend({
    ...
    save: function() {
      ...
      $.getJSON(this.url, this.attributes, function(data) {
        ...
        //debugger;
        log.push('LogWorkModel.save() executed.');

      }).error( function(err) {
        //This is the error handler.
        //debugger;
        log.push('Error while trying LogWorkModel.save(). Most likely due to communication issue with the server.');
        sendLog();
        console.error('Communication error with server while execute LogWorkModel.save()');
      });
